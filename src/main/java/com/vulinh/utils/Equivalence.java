 ...
 * plays a critical role in both {@code equals()} and {@code hashCode()} calculations. This {@code
 * id} object should be <b>immutable</b> or at least <b>effectively immutable</b> after the {@link
 * Equivalence} instance is created, especially if the instance is to be used in hash-based
 * collections (e.g., {@link java.util.HashSet}, {@link java.util.HashMap}). If the state of a
 * mutable {@code id} object changes after the {@link Equivalence} instance's creation:
 *
 * <ul>
 *   <li>The pre-computed {@code hash} (cached at creation) will no longer accurately reflect the
 *       current state of the {@code id}, potentially leading to the {@link Equivalence} instance
 ...
 *
 * <p>Equality between two {@link Equivalence} instances is determined by:
 *
 * <ol>
 *   <li>Whether they are both instances of {@link Equivalence}.
...
public record Equivalence<T>(T value, Object id, EqualityDeepness equalityDeepness, int hash) {
   ...
   * methods to construct {@link Equivalence} objects with default or specified {@link
   * EqualityDeepness}.
   */
  public static class Creator {
     ...
     *     null. The object returned by this extractor (the ID) should ideally be <b>immutable or
     *     effectively immutable</b> to prevent issues with pre-computed hash codes and equality
     *     checks (see class-level caution on {@link Equivalence}).
     ...
     *
     * <p>Distinctness is determined by comparing an ID extracted from each object, using a
     * specified {@link EqualityDeepness}.
     *
     ...
     *   <li>Applying {@link java.util.stream.Stream#distinct()} to the stream of {@code
     *       Equivalence} objects. This operation uses the {@code Equivalence} class's {@code
     *       equals} and {@code hashCode} methods, thereby ensuring uniqueness based on the
     *       extracted IDs.
     *   <li>Unwrapping each distinct {@link Equivalence} object back to its original value (of type
     *       {@code T}).
     * </ul>
     *
     * <p>The resulting stream contains the original objects, but with duplicates (as defined by
     ...
     *     by this extractor (the ID) should ideally be <b>immutable or effectively immutable</b>
     *     (see class-level caution on {@link Equivalence}).
     * @param equalityDeepness The strategy for comparing IDs and calculating their hash codes. Must
     *     not be null.
